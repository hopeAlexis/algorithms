...
#include <fstream>
#include <string>
#include <unordered_map>    //библиотека дл€ хеш-таблиц

int hashFunction(const std::string& key, int n) //функци€, котора€ создаЄт "рандомные" ключи дл€ хеш-таблицы
{
    int sum = 0;
...
{
    std::ifstream F(key, std::ios::in); //экземлп€р класса ifstream
    int count = 0;
    char temp[10000];
...
int main()
{
    ...
    while (std::getline(fin, line)) //getline позвол€ет нам считать строку с пробелами
    {

...
Element* treeMaker(int data, Element* root) //создаЄм элемент бинарного дерева
{
...
void addElement(int data, Element*& root)   //добавл€ем элемент в дерево
{
    if (root == nullptr)
    {
    ...
    {
        if (data < r->value)    //если меньше значени€ - становитс€ левым ребЄнком
            r = r->left;
        else
            r = r->right;   //если больше значени€ - становитс€ правым ребЄнком
    }
    if (data == r->value)
        return;
...
void fillTree(std::string& bracketEntry, int& i, Element*& root)    //заполнен€ем дерево скобочной записью
{
    int value = 0;
...
int depth(int data, Element* v, int k)  //как глубоко элемент находитс€ в дереве
{
    if (v == nullptr)
    {
        return -1;
...
//рекурсивные обходы
void straightPass(Element* root)    //пр€мой
{
    if (root == nullptr)
        return;
    std::cout << root->value << " ";
...
void nonRecursivePass(Element* root)
{
    ...
    stack.push(root);   //добавл€ем в стек корень
    while (!stack.empty())
    {
        Element* node = stack.top();
        stack.pop();
        std::cout << node->value << " ";
        if (node->left != nullptr) //провер€ем существовани€ левого поддерева
        {
            stack.push(node->left);
        }
        if (node->right != nullptr) //провер€ем существовани€ правого поддерева
        {
...
int main()
{
    Element* root = nullptr;    //пустое значение корн€
    std::string bracketEntry;   //скобочна€ запись
